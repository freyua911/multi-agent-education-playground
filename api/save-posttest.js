...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error) {
      ...
      res.status(500).json({ error: 'Failed to save posttest', detail: error.message })
      return
    }

    res.status(200).json({ ok: true, posttestId: data.id })
  } catch (error) {
    console.error('save-posttest error:', error)
